...
Deno.test(
  "runPublishStage: an already-completed job short-circuits to already_published without claiming",
  async () => {
...
Deno.test(
  "runPublishStage: a job never approved (still at an earlier stage) is not_claimed",
  async () => {
    const client = new FakeSupabaseClient();
...
Deno.test(
  "runPublishStage: no_current_draft fails the job when the claimed job has no draft",
  async () => {
    const client = new FakeSupabaseClient();
...
Deno.test(
  "runPublishStage: happy path calls publish_recipe_draft with the resolved slug and lock token, and reports the new recipe",
  async () => {
    const client = new FakeSupabaseClient();
...
Deno.test(
  "runPublishStage: an RPC failure maps to the matching outcome and fails the job",
  async () => {
    const client = new FakeSupabaseClient();
    const { jobId } = seedReadyJob(client);
    ...
    client.onRpc("publish_recipe_draft", () => ({
      ...
      error: {
        message:
          'PUBLISH_SLUG_ALREADY_USED: slug "firinda-kabak-dolmasi" is already used by an existing recipe',
      },
    }));
...
Deno.test(
  "runPublishStage: a retryable RPC failure (missing assets) leaves the job retryable",
  async () => {
    const client = new FakeSupabaseClient();
    const { jobId } = seedReadyJob(client);
    seedDraft(client, jobId);
...
Deno.test(
  "runPublishStage: the RPC's own defensive already-published reply is passed through",
  async () => {
    const client = new FakeSupabaseClient();
    const { jobId } = seedReadyJob(client);
    seedDraft(client, jobId);
...
Deno.test(
  "runPublishStage: a genuine publish transition triggers the F0-24 nutrition recalc as a best-effort side effect",
  async () => {
    const client = new FakeSupabaseClient();
    const { jobId } = seedReadyJob(client);
    seedDraft(client, jobId);
    ...
    client.seed("recipes", [
      { id: recipeId, status: "published", servings: 4, nutrition_input_hash: null },
    ]);
    let nutritionCalls = 0;
...
Deno.test(
  "runPublishStage: a nutrition recalc failure never fails an otherwise-successful publish",
  async () => {
    const client = new FakeSupabaseClient();
    const { jobId } = seedReadyJob(client);
    seedDraft(client, jobId);
