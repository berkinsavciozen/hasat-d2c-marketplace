...
function registerHappyPathRpcs(client: FakeSupabaseClient) {
  ...
  client.onRpc("validate_recipe_slug", () => ({
    data: { valid: true, issues: [], slug: SLUG },
    error: null,
  }));
...
function seedAsset(
  ...
  const dims =
    assetType === "hero" ? { width_px: 878, height_px: 494 } : { width_px: 878, height_px: 878 };
...
Deno.test(
  "runFinalizeStage: no_approved_qa_result when the latest QA result isn't approved",
  async () => {
    const client = new FakeSupabaseClient();
    const jobId = seedFinalizeJob(client);
    const draftId = seedDraft(client, jobId, 1);
...
Deno.test(
  "runFinalizeStage: stale_qa_version when QA approved an older draft version than the current one",
  async () => {
    const client = new FakeSupabaseClient();
    const jobId = seedFinalizeJob(client);
...
Deno.test(
  "runFinalizeStage: blocking_issues_remain when the QA row still carries blocking issues",
  async () => {
    const client = new FakeSupabaseClient();
    const jobId = seedFinalizeJob(client);
    const draftId = seedDraft(client, jobId, 1);
...
Deno.test(
  "runFinalizeStage: safety_review_incomplete when a required safety finding is missing",
  async () => {
    const client = new FakeSupabaseClient();
    const jobId = seedFinalizeJob(client);
    const draftId = seedDraft(client, jobId, 1);
...
Deno.test(
  "runFinalizeStage: postgres_validation_failed when a re-run structure/crop check fails",
  async () => {
    const client = new FakeSupabaseClient();
...
Deno.test(
  "runFinalizeStage: missing_image_assets when the square variant was never stored",
  async () => {
    const client = new FakeSupabaseClient();
...
Deno.test(
  "runFinalizeStage: invalid_image_assets when a stored asset doesn't match the Step 09 contract",
  async () => {
    const client = new FakeSupabaseClient();
    registerHappyPathRpcs(client);
    const jobId = seedFinalizeJob(client);
...
Deno.test(
  "runFinalizeStage: a warning validation_status (frame suspicion) does not block finalization",
  async () => {
    const client = new FakeSupabaseClient();
    registerHappyPathRpcs(client);
    const jobId = seedFinalizeJob(client);
...
Deno.test(
  "runFinalizeStage: unresolved_stage_error when the most recent upstream stage run failed",
  async () => {
    const client = new FakeSupabaseClient();
    registerHappyPathRpcs(client);
    const { jobId } = seedFullyReadyJob(client);
...
Deno.test(
  "runFinalizeStage: finalize's own earlier failed attempt never self-blocks a later retry",
  async () => {
    const client = new FakeSupabaseClient();
    registerHappyPathRpcs(client);
    const { jobId } = seedFullyReadyJob(client);
...
Deno.test(
  "runFinalizeStage: successful finalization moves the job to stage=awaiting_approval",
  async () => {
    const client = new FakeSupabaseClient();
    registerHappyPathRpcs(client);
...
Deno.test(
  "runFinalizeStage: duplicate invocation after a successful finalization is a safe not_claimed",
  async () => {
    const client = new FakeSupabaseClient();
    registerHappyPathRpcs(client);
    const { jobId } = seedFullyReadyJob(client);
