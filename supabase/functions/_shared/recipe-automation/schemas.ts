 ...
 */
export const recipeErrorPayloadSchema = z
  .object({
...
export const recipeBatchInputSchema = z
  .object({
...
export const recipeBriefSchema = z
  .object({
    briefId: uuidSchema,
    batchId: uuidSchema,
...
export const recipePlanBatchSchema = z
  .object({
    batchId: uuidSchema,
  ...
  .strict()
  .refine((batch) => batch.briefs.every((brief) => brief.batchId === batch.batchId), {
    message: "every brief.batchId must match the batch's own batchId",
    path: ["briefs"],
  })
  .refine(
...
export const recipeIngredientDraftSchema = z
  ...
  .strict()
  .refine((ingredient) => ingredient.crop !== null || ingredient.freeTextName !== null, {
    message: "either crop or freeTextName must be present",
    path: ["freeTextName"],
  });
...
export const recipeDraftPayloadSchema = z
  .object({
    jobId: uuidSchema,
    ...
    dietTags: z.array(nonEmptyTrimmedString).default([]),
    /** Empty means human-reviewed no controlled allergen; null/free text is never publishable. */
  ...
  .refine(
    ...
    {
      message: "steps must have sequential step_no starting at 1, with no gaps or duplicates",
      path: ["steps"],
    },
  );
...
export const recipeSafetyReviewSchema = z
  .object({
    temperature: safetyFindingSchema,
    timing: safetyFindingSchema,
    allergens: safetyFindingSchema
      .extend({
  ...
  .refine(
    ...
    {
      message: "approved can only be true once reviewedBy and reviewedAt are set",
      path: ["approved"],
    },
  );
 ...
 */
export const recipeQAIssueSchema = z
  .object({
    code: z.string().regex(/^[A-Z][A-Z0-9_]*$/, "code must be SCREAMING_SNAKE_CASE"),
...
export const RECIPE_QA_DECISION_VALUES = [
  "approved",
  "revision_required",
...
export const recipeQAResultSchema = z
  .object({
    jobId: uuidSchema,
    ...
    scores: z
      ...
      .strict(),
    blockingIssues: z
      .array(recipeQAIssueSchema.extend({ severity: z.literal("blocking") }).strict())
      .default([]),
    nonBlockingSuggestions: z
      .array(recipeQAIssueSchema.extend({ severity: z.enum(["info", "warning"]) }).strict())
      .default([]),
  ...
  .strict()
  .refine((qa) => qa.decision !== "approved" || qa.blockingIssues.length === 0, {
    message: "decision cannot be 'approved' while blockingIssues is non-empty",
    path: ["decision"],
  })
  .refine((qa) => qa.blockingIssues.length === 0 || !qa.approvedForImaging, {
    message: "approvedForImaging cannot be true while blockingIssues is non-empty",
    path: ["approvedForImaging"],
  })
  .refine(
    (qa) =>
      qa.approvedForImaging === (qa.decision === "approved" && qa.blockingIssues.length === 0),
    {
      message:
        "approvedForImaging must be true iff decision is 'approved' and there are no blockingIssues",
      path: ["approvedForImaging"],
    },
  );
...
export const recipeImageSpecSchema = z
  .object({
    ...
    cropTargets: z
      .array(recipeImageCropTargetSchema)
      .min(1)
      .max(2)
  ...
  .strict()
  .refine((spec) => spec.sourceWidthPx === spec.sourceHeightPx, {
    message: "source image must be square (sourceWidthPx must equal sourceHeightPx)",
    path: ["sourceHeightPx"],
  });
...
export const recipeStageResultSchema = z
  .object({
    jobId: uuidSchema,
    batchId: uuidSchema.nullable().default(null),
    recipeId: uuidSchema.nullable().default(null),
