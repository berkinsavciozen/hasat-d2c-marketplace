...
| `job-detail.ts` | `loadJobDetail()` — the draft detail view: current draft content/ingredients/steps (reused from `../qa/context.ts`'s `loadCurrentDraft`), Postgres RPC validation (reused from `../writer/validate-draft.ts`), the latest full QA result, both images with public URLs and frame-suspicion warnings, revision history across every draft version, recent stage-run history, this job's own admin-review audit trail, and (DQ-2 §4) `draftQualityIssues` — `quality.ts`'s `getDraftQualityIssues` over the current draft, advisory only; a failing check sets `draftQualityCheckFailed` instead of failing the whole view. |
