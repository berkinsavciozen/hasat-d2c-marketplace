...
MIGRATION="$REPO_ROOT/supabase/migrations/20260921084620_ux1c0_disable_private_step_photo_writes.sql"
CONTAINER="hasat-ux1c0-pg17-$RANDOM"

cleanup() { docker rm -f "$CONTAINER" >/dev/null 2>&1 || true; }
trap cleanup EXIT

# Production preflight: canonical repository versions match the already-applied
# production records. Never replay or repair either production history entry here.
test -f "$REPO_ROOT/supabase/migrations/20260918091532_ux1b_private_step_photo_preservation.sql"
test ! -e "$REPO_ROOT/supabase/migrations/20260918090000_ux1b_private_step_photo_preservation.sql"
...
docker exec -i "$CONTAINER" psql -v ON_ERROR_STOP=1 -X -U postgres -d ux1c0 < "$REPO_ROOT/supabase/migrations/20260918091532_ux1b_private_step_photo_preservation.sql"
