...
import { LifecycleBadge } from "@/components/hasat/LifecycleBadge";
import { statusVisual } from "@/lib/hasat/offer-status";
...
function Negotiation() {
  ...
  if (isPending || isFetching || !offers)
    return (
      <div className="p-8">
        <LoadingDots />
      </div>
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          <div className="text-xs opacity-70">
            ...
            {(() => {
              const slug = offer.buyerName
                ? slugifyFarmer(offer.buyerName) || offer.producerId
                : offer.producerId;
              return slug ? (
                <Link to="/s/$slug" params={{ slug }} className="underline hover:text-saffron">
                  {offer.buyerName}
                </Link>
              ) : (
                offer.buyerName
      ...
      <div className="p-4 md:p-8 max-w-3xl mx-auto md:mx-0 space-y-5 pb-32">
        <section className="rounded-2xl border bg-card p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h2 className="truncate font-medium">{formatCrop(offer.crop)}</h2>
              <p className="truncate text-xs text-hmuted">{offer.buyerName}</p>
              <p className="mt-2 text-sm tabular-nums">
                {formatQuantity(offer.quantity, offer.unit)} {offer.unit} ·{" "}
                <strong>{formatTRY(counterTotal)}</strong>
              </p>
            </div>
            <LifecycleBadge
              tone={
        ...
        <div className="rounded-2xl border p-4 bg-card">
          ...
          <div
            className={`font-mono text-xl mt-1 ${diff > 0 ? "text-hred" : diff < 0 ? "text-sage" : ""}`}
          >
            {diff > 0 ? "+" : ""}
            {formatTRY(diff)}
          </div>
        ...
        <div className="fixed inset-x-0 bottom-[calc(56px+max(env(safe-area-inset-bottom),0.5rem))] z-30 grid grid-cols-3 gap-1.5 border-t bg-background p-3 md:static md:border-0 md:bg-transparent md:p-0 md:gap-2">
          <Button
            variant="outline"
            onClick={reject}
            className="border-hred/40 text-hred hover:bg-hred/5 px-2 text-xs sm:text-sm"
          >
            Reddet
          </Button>
          <Button
            ...
            className="border-primary px-2 text-xs text-primary sm:text-sm"
          >
            <span className="sm:hidden">Karşı Teklif</span>
            <span className="hidden sm:inline">Karşı Teklif Yap</span>
          </Button>
          <Button
            onClick={accept}
...
function SideCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-4 ${highlight ? "border-saffron bg-saffron/5" : "bg-card"}`}
    >
      ...
      <div className="mt-3 space-y-2 text-sm">
        <Row
          ...
          cls={diffCls(!!original && snap.quantity !== original.quantity)}
        />
        <Row
          label="Birim Fiyat"
          value={`${formatTRY(snap.pricePerUnit)}/${unit}`}
          cls={diffCls(!!original && snap.pricePerUnit !== original.pricePerUnit)}
        />
        <Row
          label="Teslimat"
          value={snap.delivery ?? "—"}
          cls={diffCls(!!original && snap.delivery !== original.delivery)}
        />
        <Row
          label="Teslim Tarihi"
          value={snap.deliveryDate || "—"}
          cls={diffCls(!!original && snap.deliveryDate !== original.deliveryDate)}
        />
        {snap.note && (
          <div
            className={`mt-2 rounded-lg bg-muted/60 p-2 text-xs italic ${diffCls(!!original && snap.note !== original.note)}`}
          >
            "{snap.note}"
          </div>
        )}
      ...
      <div className="mt-4 border-t pt-3">
        ...
        <div className={`font-mono text-2xl font-semibold ${highlight ? "text-saffron" : ""}`}>
          {formatTRY(total)}
        </div>
      </div>
    </div>
