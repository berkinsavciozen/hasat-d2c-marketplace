      ...
      <div className="px-4 md:px-8 py-5 pb-32 md:pb-5">
        ...
        <div className="mt-3 rounded-xl border bg-card p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <a
              ...
              className="inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] text-primary hover:bg-primary/10"
            >
            ...
            {active.length > 0 ? (
              <div className="flex gap-1.5 overflow-x-auto p-2 bg-card">
                {active.slice(0, 6).map((l) => (
                  <div key={l.id} className="shrink-0 w-24">
                    <div className="aspect-[4/3] w-24 rounded-lg bg-cream overflow-hidden grid place-items-center text-2xl">
                      {l.photos?.[0] ? (
                        <img
                          src={l.photos[0]}
          ...
          <div className="mt-2 truncate text-[11px] text-hmuted">{vUrl}</div>
        </div>
          ...
          <TabsContent value="active" className="mt-4 space-y-3">
            {isLoading ? (
              ...
              <div className="rounded-2xl border border-dashed py-12 text-center">
                ...
                <div className="mb-4 text-xs text-hmuted">
                  Hasadınızı vitrine ekleyerek alıcılarla buluşun.
                </div>
                <button
            ...
            {isLoading ? (
              ...
              <div className="rounded-2xl border border-dashed py-12 px-6 text-center">
                ...
                <div className="text-xs text-hmuted">
                  Henüz taslak yok. Bir parsel oluşturduğunuzda, o parselin her ürünü için otomatik
                  bir taslak ilan hazırlanır.
                </div>
              </div>
            ) : (
              drafts.map((l) => (
        ...
        <section className="mt-8">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-lg font-semibold">Tarlalarım</h2>
            <Link to="/farmer/settings" className="text-xs text-primary underline">
              Düzenle
            </Link>
          </div>
          {parcels.length === 0 ? (
              ...
              {parcels.map((p) => (
                <div key={p.id} className="rounded-2xl border bg-card p-4">
                  <div className="flex items-baseline justify-between gap-2">
                    ...
                    <div className="text-xs text-hmuted truncate min-w-0 text-right">
      ...
      <AlertDialog open={!!confirmDelete} onOpenChange={(o) => !o && setConfirmDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {formatCrop(confirmDelete?.crop)} ilanınız vitrinden kaldırılacak. Bu işlem geri
              alınamaz.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function ListingCard({
  ...
  return (
    <div
      className={`rounded-2xl border bg-card p-4 ${muted ? "opacity-60" : ""} ${draft ? "border-dashed" : ""}`}
    >
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {listing.batchName?.trim() && (
              <span className="rounded-full bg-cream px-2 py-0.5 text-[10px] font-medium text-hmuted">
                📦 {listing.batchName}
              </span>
            )}
            <span
          ...
          {parcelLabel && <div className="mt-0.5 text-[11px] text-hmuted">📍 {parcelLabel}</div>}
          <div className="mt-0.5 text-xs text-hmuted">
            {listing.quantity} {listing.unit} • Min. {listing.minOrder} {listing.unit}
          </div>
        ...
        <div className="mt-3 flex gap-2">
          {!draft && (
            <Link
              ...
              className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border py-2 text-xs font-medium hover:bg-cream"
            >
              📦 Parti
            </Link>
          )}
          <button
            onClick={onEdit}
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-medium ${draft ? "bg-saffron text-white hover:opacity-90" : "border hover:bg-cream"}`}
          >
            {draft ? (
              <>✓ Yayınla</>
            ) : (
          ...
          <button
            onClick={onRemove}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-hred/30 py-2 text-xs font-medium text-hred hover:bg-hred/5"
          >
...
function ListingSheet({
  ...
  useEffect(() => {
    if (editing) {
      ...
      setPrice(editing.pricePerUnit);
      setMinOrder(editing.minOrder);
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        ...
        toast.success(
          status === "draft" ? "Yeni batch taslak olarak kaydedildi" : "Ürün yayınlandı",
        );
      }
      onClose();
    } catch (e: any) {
          ...
          {!editing && (
            <>
              <div>
                ...
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        parcels.length === 0 ? "Önce Ayarlar'dan parsel ekleyin" : "Parsel seçin"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={cropsLoading ? "Yükleniyor…" : "Ürün seçin"} />
                  </SelectTrigger>
                  <SelectContent>
                    {cropOptions.map((o) => (
            ...
            <>
              {batchDecisionOverride && (
                <div className="rounded-lg bg-cream/70 px-3 py-2 text-[11px] text-hmuted">
                  Bu, aynı parsel+ürün için yeni bir batch. Taslak olarak kaydedilecek; hazır
                  olduğunda “Yayınla” ile aktif hale getirebilirsin.
                </div>
              )}
              {editing && (
                <div>
              ...
              <div>
                ...
                <Stepper
                  ...
                  unit={unit}
                  units={["g", "kg", "L"]}
                  onUnitChange={(u) => setUnit(u as "g" | "kg" | "L")}
                />
              </div>
              <div>
  ...
  return (
    <div className="rounded-xl border border-dashed p-3 space-y-2 bg-cream/40">
      ...
      {listingId ? (
        ...
        <div className="text-[11px] text-hmuted">
          Yayınladıktan sonra Günlük'ten hasat kayıtlarını bağlayarak izlenebilirliği
